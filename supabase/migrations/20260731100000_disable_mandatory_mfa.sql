-- Temporarily allow password-authenticated staff sessions to use their assigned
-- role permissions. TOTP enrollment data is preserved so MFA can be re-enabled
-- with a future forward-only migration for a clinic deployment.
...
comment on function private.has_permission(text) is
  'Checks active hospital role permissions. Mandatory MFA is temporarily disabled.';
